...
static esp_err_t esp_lcd_touch_ft5x06_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    err = touch_ft5x06_i2c_read(tp, FT5x06_TOUCH_POINTS, points, 2);
    ESP_RETURN_ON_ERROR(err, TAG, "I2C read error!");

    if (points[1] > 5 || points[1] == 0) {
        return ESP_OK;
    }

    /* Number of touched points */
    points[1] = (points[1] > CONFIG_ESP_LCD_TOUCH_MAX_POINTS ? CONFIG_ESP_LCD_TOUCH_MAX_POINTS : points[1]);

    err = touch_ft5x06_i2c_read(tp, FT5x06_TOUCH1_XH, data, 7 * points[1]);
    ESP_RETURN_ON_ERROR(err, TAG, "I2C read error!");

    portENTER_CRITICAL(&tp->data.lock);

    /* Number of touched points */
    tp->data.points = points[1] & 0x0F;

    /* Fill all coordinates */
    for (i = 0; i < points[1]; i++) {
        tp->data.coords[i].x = (((uint16_t)data[(i * 7) + 2] & 0x0f) << 8) + data[(i * 7) + 3];
        tp->data.coords[i].y = (((uint16_t)data[(i * 7) + 4] & 0x0f) << 8) + data[(i * 7) + 5];
    }
